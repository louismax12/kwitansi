...
class DashboardController {
    ...
    public function index() {
        ...
        $res1 = $this->conn->query("SELECT SUM(total_bayar) as total FROM kwitansi_cetak WHERE tanggal_transaksi LIKE '$monthEscaped'");
        $row1 = $res1 ? $res1->fetch_assoc() : null;
        $totalPendapatan = ($row1 && $row1['total']) ? $row1['total'] : 0;

        // Get total pelanggan
        $res2 = $this->conn->query("SELECT COUNT(*) as count FROM kwitansi_pelanggan");
        $row2 = $res2 ? $res2->fetch_assoc() : null;
        $totalPelanggan = $row2 ? $row2['count'] : 0;

        // Get total kwitansi
        $res3 = $this->conn->query("SELECT COUNT(*) as count FROM kwitansi_cetak");
