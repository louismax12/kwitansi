...
@media print{.btn-print-bar{display:none}}
@page{margin:0}
...
<div class="tbl-wrap">
  <table class="grid">
    ...
    <tr class="<?= ($editData && $editData['no_kwitansi']===$k['no_kwitansi']) ? 'selected' : '' ?>">
      ...
      <td><?= htmlspecialchars(mb_substr($k['untuk_pembayaran'],0,40)) ?><?= mb_strlen($k['untuk_pembayaran'])>40?'…':'' ?></td>
      <td><?= htmlspecialchars(mb_substr($k['uang_sejumlah'],0,44)) ?>…</td>
      <td><?= htmlspecialchars($k['tgl']) ?></td>
      <td><?= htmlspecialchars($k['kasir']) ?></td>
      <td style="white-space:nowrap">
        <a href="index.php?action=edit&id=<?= urlencode($k['no_kwitansi']) ?>"   class="btn btn-edit">E</a>
        <a href="index.php?action=print&id=<?= urlencode($k['no_kwitansi']) ?>"  class="btn btn-print" target="_blank">P</a>
        <a href="index.php?action=delete&id=<?= urlencode($k['no_kwitansi']) ?>" class="btn btn-delete"
           onclick="return confirm('Hapus kwitansi <?= htmlspecialchars($k['no_kwitansi']) ?>?')">X</a>
      </td>
    </tr>
